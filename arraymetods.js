...
function against an accumulator and each value of the array(from right - to - left) to reduce it to a single value.
``
`javascript
    let arr = [1, 2, 3, 4];
...
22. ** `sort(compareFunction)` **
    -Sorts the elements of an array in place and returns the array.The
...
24. ** `flatMap(callback(element, index, array))` **
    -First maps each element using a mapping
...
These methods provide a comprehensive toolkit
for working with arrays in JavaScript, enabling you to perform a wide variety

of operations efficiently and effectively.